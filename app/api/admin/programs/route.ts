...
export async function POST(request: NextRequest) {
  try {
    ...
    const id = await AdminDatabaseService.createProgram(
      text.trim(),
      goalId,
      categoryId,
      pillarId,
...
export async function PUT(request: NextRequest) {
  try {
    ...
    await AdminDatabaseService.updateProgram(id, updateData);
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (bulk === 'true') {
      ...
      const deletedCount = await AdminDatabaseService.bulkDelete('strategic_programs', ids);
      return NextResponse.json({ 
    ...
    await AdminDatabaseService.deleteProgram(id);
